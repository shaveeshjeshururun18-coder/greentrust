...
const SparkleButton: React.FC<SparkleButtonProps> = ({ onClick, children, className = '' }) => {
    ...
    const handleClick = (e: React.MouseEvent) => {
        setIsBursting(true);
        onClick(e);
        setTimeout(() => setIsBursting(false), 500); // Reset after animation
    };

    return (
        <button
            onClick={handleClick}
            className={`sparkle-btn group relative overflow-visible ${isBursting ? 'bursting' : ''} ${className}`}
        >
            {/* Sparkle SVGs */}
            ...
            {/* Button text - shadow layer */}
            <span className="sparkle-text-shadow">{children}</span>
            {/* Button text - glare layer */}
